...
type LayerMethod =
  | "rotateTopCW" | "rotateTopCCW"
  | "rotateXMidCW" | "rotateXMidCCW"
  | "rotateBottomCW" | "rotateBottomCCW"
  | "rotateLeftCW" | "rotateLeftCCW"
  | "rotateYMidCW" | "rotateYMidCCW"
  | "rotateRightCW" | "rotateRightCCW"
  | "rotateFrontCW" | "rotateFrontCCW"
  | "rotateZMidCW" | "rotateZMidCCW"
...
class CubeView {
  ...
  private readonly COLORS: Record<Face, string> = {
    Y: "#ffd43b", R: "#d92b3c", B: "#2256d6", G: "#1eaa5b", O: "#ff7a18", W: "#f4f4f0",
  }
  ...
  private createCubie(cube: Cube): CubieEntry {
    ...
    el.style.cssText =
      "position:absolute; left:0; top:0; width:" + S + "px; height:" + S + "px; transform-style:preserve-3d;"
    ...
    this.DIRS.forEach((dir) => {
      ...
      face.style.cssText =
        "position:absolute; left:0; top:0; width:" + S + "px; height:" + S + "px; background:#14141b; border-radius:9px; transform:" +
        faceTf[dir] + "; -webkit-backface-visibility:hidden; backface-visibility:hidden;"
  ...
  private renderCube() {
    ...
    this.entries.forEach((e) => {
      ...
      e.el.style.transform =
        "translate3d(" + ((p.X - 1) * U - H) + "px," + ((p.Z - 1) * U - H) + "px," + ((p.Y - 1) * U) + "px)"
      this.DIRS.forEach((dir) => {
        const col = e.cube.orientation[dir]
        const st = e.stickers[dir]
        if (col) { st.style.display = "block"; st.style.background = this.COLORS[col] }
  ...
  private orbitStr() { return "rotateX(" + this.pitch + "deg) rotateY(" + this.yaw + "deg)" }

  private processQueue() {
    if (this.animating || !this.queue.length || !this.worldEl) return
    const it = this.queue.shift()!
    if (it.kind === "cube") { this.animateCube(it.c); return }
  ...
  private animateLayer(m: MoveDef, angle: number, method: LayerMethod, opts: MoveOpts) {
    this.animating = true
  ...
  private afterMove(opts: MoveOpts) {
    if (opts.scramble) {
      this.scrambleLeft--
      if (this.scrambleLeft <= 0) { this.status = "ready"; this.updateStatus() }
      return
    }
    if (opts.count !== false) {
      this.startTimer()
  ...
  private startTimer() {
    ...
    this.status = "solving"; this.updateStatus()
    this.timer = window.setInterval(() => { this.elapsed = performance.now() - this.t0; this.updateStats() }, 60)
  }

  private stopTimer() {
    if (this.timer) clearInterval(this.timer)
    this.running = false
    if (this.t0) { this.elapsed = performance.now() - this.t0; this.updateStats() }
  }
  ...
  private scramble() {
    ...
    this.status = "scrambling"; this.moveCount = 0; this.elapsed = 0
    this.updateStatus(); this.updateStats()
    const faces: (MoveKey)[] = ["U", "D", "L", "R", "F", "B"]
    const seq: Array<{ f: MoveKey; prime: boolean }> = []
    let lastAxis: Axis | "" = ""
    for (let i = 0; i < 24; i++) {
      let f: MoveKey; let guard = 0
      do { f = faces[Math.floor(Math.random() * faces.length)]; guard++ }
  ...
  private reset() { this.doReset(true) }
  ...
  private doReset(resetView: boolean) {
    ...
    if (resetView) { this.hasScrambled = false; this.yaw = -34; this.pitch = -22; this.applyView(true) }
    this.status = "ready"; this.moveCount = 0; this.elapsed = 0
    this.updateStatus(); this.updateStats()
  }

  // ---------- view ----------
  private resetView() { this.yaw = -34; this.pitch = -22; this.applyView(true) }

  // ---------- pointer ----------
  private onDown(e: PointerEvent) {
    this.sceneEl.setPointerCapture && this.sceneEl.setPointerCapture(e.pointerId)
    this.px = e.clientX; this.py = e.clientY
    this.yaw0 = this.yaw; this.pitch0 = this.pitch
    this.dragging = true; this.turnCommitted = false
  ...
  private onUp(_e: PointerEvent) {
    this.dragging = false; this.dragFace = null
  ...
  private pickTurn(faceEl: HTMLElement, dx: number, dy: number): { face: MoveKey; prime: boolean } | null {
    ...
    axes.forEach((axis) => {
      const vec: Vec3 = { X: 0, Y: 0, Z: 0 }; vec[axis] = 1
    ...
    const m: Vec3 = { X: 0, Y: 0, Z: 0 }; m[chosen.axis] = chosen.sign
    // r = normal x m
    const r: Vec3 = {
  ...
  private onKey(e: KeyboardEvent) {
    ...
    const k = e.key
    if (k === "ArrowLeft") { this.cubeMove("spinLeft"); e.preventDefault(); return }
    if (k === "ArrowRight") { this.cubeMove("spinRight"); e.preventDefault(); return }
    if (k === "ArrowUp") { this.cubeMove("rollUp"); e.preventDefault(); return }
    if (k === "ArrowDown") { this.cubeMove("rollDown"); e.preventDefault(); return }
    const map: Record<string, MoveKey> = {
      u: "U", d: "D", l: "L", r: "R", f: "F", b: "B", m: "M", e: "E", s: "S",
    }
    const face = map[k.toLowerCase()]
    if (face) { this.move(face, e.shiftKey); e.preventDefault() }
  }
  ...
  private updateStatus() {
    ...
    if (dot) { dot.style.background = st.color; dot.style.boxShadow = "0 0 10px " + st.color }
