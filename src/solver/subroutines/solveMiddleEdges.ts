...
export async function solveMiddleEdges(solver: RubiksCubeSolver) {
    const bottomEdge = solver.rubiks.cubes.find(
        (cube) => cube.isInBottomLayer && !cube.hasFace('W')
    )
...
async function solveBottomEdge(solver: RubiksCubeSolver, bottomEdge: Cube) {
    ...
    while (
        bottomEdge.orientation.front !== solver.cubeMap.get(positionMap[17])?.orientation.front
    ) {
