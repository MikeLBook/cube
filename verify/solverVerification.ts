...
function edgesSolved(): boolean {
  const c = centers();
  const e2 = get(2), e4 = get(4), e6 = get(6), e8 = get(8);
  return !!(
    e2?.orientation.top === "Y" && e2?.orientation.back === c.back &&
    e4?.orientation.top === "Y" && e4?.orientation.left === c.left &&
    e6?.orientation.top === "Y" && e6?.orientation.right === c.right &&
    e8?.orientation.top === "Y" && e8?.orientation.front === c.front
  );
}
// Independent ground truth: all 4 top corners are Y-on-top with both sides matching centers.
function cornersSolved(): boolean {
  const c = centers();
  const c1 = get(1), c3 = get(3), c7 = get(7), c9 = get(9);
  return !!(
    c1?.orientation.top === "Y" && c1?.orientation.left === c.left && c1?.orientation.back === c.back &&
    c3?.orientation.top === "Y" && c3?.orientation.right === c.right && c3?.orientation.back === c.back &&
    c7?.orientation.top === "Y" && c7?.orientation.left === c.left && c7?.orientation.front === c.front &&
...
const MOVES = [
  ...
  "rotateLeftCW", "rotateLeftCCW", "rotateRightCW", "rotateRightCCW",
  "rotateFrontCW", "rotateFrontCCW", "rotateBackCW", "rotateBackCCW",
];
...
async function count(n: number) {
  ...
  for (let i = 0; i < n; i++) {
    let r: string;
    try { r = await runSeq(randSeq(50)); }
    catch (e: any) { r = "threw:" + e.message; }
    tally[r] = (tally[r] || 0) + 1;
  }
